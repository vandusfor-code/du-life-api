...
// Registro de envíos en vuelo. En Vercel la función serverless se congela
// apenas se envía la respuesta HTTP: un push disparado sin await (para no
// demorar la respuesta de WhatsApp) muere a mitad de camino y la notificación
// nunca sale hacia FCM/APNs — este era el motivo de "se activan pero no
// llegan". Los ejecutores siguen disparando sin esperar, y el handler del
// job llama esperarPushesPendientes() justo antes de responder, dándole a
// los envíos el tiempo que necesitan sin sumar latencia al usuario.
const pushesEnVuelo = new Set();

export async function esperarPushesPendientes() {
  while (pushesEnVuelo.size > 0) {
    await Promise.allSettled([...pushesEnVuelo]);
  }
}
 ...
 */
export function enviarNotificacionPush(usuarioId, payload) {
  const promesa = _enviarNotificacionPush(usuarioId, payload);
  pushesEnVuelo.add(promesa);
  promesa.finally(() => pushesEnVuelo.delete(promesa));
  return promesa;
}

async function _enviarNotificacionPush(usuarioId, payload) {
  try {
