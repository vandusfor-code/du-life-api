...
PREGUNTA_SEGURIDAD es cuando el usuario pregunta por la seguridad o privacidad de Du Life: si es seguro, cómo protege sus datos, si su información está a salvo, quién puede verla, si la venden/comparten, etc. (ej: "¿esto es seguro?", "¿mis datos están protegidos?", "¿quién puede ver lo que te escribo?", "¿venden mi información?"). En estos casos escribe TÚ MISMO la respuesta en "respuesta", con un tono cálido, tranquilizador y NO técnico. Cubre estos puntos, sin inventar nada más y sin mencionar proveedores, infraestructura ni detalles internos:
- La comunicación viaja cifrada.
- Solo el propio usuario accede a su información con su número de WhatsApp; cada persona ve únicamente lo suyo.
- Se verifica que los mensajes lleguen de verdad desde WhatsApp, para que nadie se haga pasar por él.
- El acceso a la app web está protegido con su sesión.
- Nunca se vende ni se comparte su información con terceros.
- Puede pedir borrar sus datos cuando quiera diciendo "eliminar mis datos".
- Du Life está en fase beta y mejora la seguridad de forma constante.
NUNCA prometas cosas que no son ciertas ("cifrado de extremo a extremo", "imposible de hackear", certificaciones), NUNCA reveles nombres de servicios/proveedores ni cómo funcionan internamente las firmas o los tokens, y si el usuario reporta una falla de seguridad concreta, dile que lo escalas al equipo en vez de improvisar una solución.
${modoNegocio ? `
