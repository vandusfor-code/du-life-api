...
import { supabase, obtenerOCrearUsuario, actualizarUsuario, crearOActualizarEntidad, crearHecho, guardarMensaje } from './supabase.js';
import { enviarMensaje, enviarImagen } from './whatsapp.js';
import { programarJob } from './qstash.js';

// Horas de espera antes de recordarle a alguien que dejó el registro a
// medias. Suficiente para no sentirse invasivo, corto para seguir dentro
// de la ventana de 24h de servicio al cliente (no hace falta plantilla).
const HORAS_RECORDATORIO_ONBOARDING = 3;

const PWA_IMAGEN_URL = 'https://wfanfkckbczzyxrubxeu.supabase.co/storage/v1/object/public/assets/069d7a9f-bbbe-43eb-8105-44419456e606.jpg';
...
export async function procesarOnboarding(telefono, mensajeTexto, nombreContacto) {
  ...
  switch (estado.paso_actual) {
    case 1: respuesta = await paso1(usuario, estado); break;
    case 2: respuesta = await paso2(usuario, estado, mensajeTexto); break;
    case 3: respuesta = await paso3(usuario, estado, mensajeTexto, pais); break;
    case 4: respuesta = await paso4(usuario, estado, mensajeTexto, pais); break;
    case 5: respuesta = await paso5(usuario, estado, mensajeTexto, pais); break;
    default: respuesta = await finalizar(usuario, estado, pais); break;
  }

  // Si el registro sigue incompleto después de este mensaje (respuesta no
  // nula: finalizar() ya devuelve null), programar un recordatorio único
  // por si la persona no vuelve a escribir. El job mismo revisa a la hora
  // de ejecutarse si ya se completó o si ya se envió antes, así que
  // programarlo repetido en cada paso no genera mensajes duplicados.
  if (respuesta) {
    const cuando = new Date(Date.now() + HORAS_RECORDATORIO_ONBOARDING * 60 * 60 * 1000);
    programarJob('recordatorio-onboarding', {
      usuario_id: usuario.id,
      telefono: usuario.telefono,
      nombre: estado.como_llamar_capturado || estado.nombre_capturado || usuario.nombre,
    }, cuando.toISOString()).catch(e => console.error('Error programando recordatorio onboarding:', e.message));
  }
...
async function finalizar(usuario, estado, pais) {
  ...
  const mensajeFinal = `¡Listo, ${comoLlamar}! 🎉\n\n` +
  ...
  // Se envían los 3 mensajes acá mismo, en orden, en vez de devolver el
  // texto para que lo mande el llamador: así garantizamos que la imagen y
  // el link del dashboard lleguen DESPUÉS del mensaje de bienvenida
  // (si se devolviera el texto, el llamador lo enviaría recién después de
  // que esta función retorne, es decir después de la imagen y el link).
  await enviarMensaje(usuario.telefono, mensajeFinal);
  await guardarMensaje(usuario.id, 'assistant', mensajeFinal, { intencion: 'onboarding' });

  await enviarImagen(
    usuario.telefono,
    PWA_IMAGEN_URL,
    'Así puedes tener Du Life siempre a mano 📱'
  );

  await enviarMensaje(
    usuario.telefono,
    '🌟 Tu dashboard personal ya está listo:\n\nhttps://app.dur.life\n\nAhí puedes ver todo lo que registras conmigo. ¡Instálala como app siguiendo los pasos de la imagen! 🙌'
  );

  // null: ya enviamos y logueamos el mensaje nosotros mismos, así que el
  // llamador (procesarMensaje) no debe reenviarlo ni loguearlo de nuevo.
  return null;
}
