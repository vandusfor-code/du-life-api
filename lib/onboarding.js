...
async function extraerApodoConClaude(mensajeTexto) {
  try {
    ...
    const extraction = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `El usuario respondió esto cuando le preguntamos cómo le gusta que le digan: "${mensajeTexto}". Extrae ÚNICAMENTE el apodo o nombre, sin comillas, sin explicación, sin puntuación. Solo la palabra. Ejemplos: "me dicen Du" → Du | "puedes llamarme Juancho" → Juancho | "Du" → Du | "todos me llaman Pipe" → Pipe`
      }]
    });

    const apodo = extraction.content[0]?.text?.trim();
    if (!apodo || apodo.length < 1 || apodo.length > 40) return null;
    return apodo;
  } catch (e) {
