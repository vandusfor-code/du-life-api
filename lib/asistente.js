...
import {
  angelEstaActivo, esMensajeCierreAngel, cerrarAngelGuardian,
  esMensajeSalida, ofrecerAngelGuardian, hayOfertaPendiente,
  limpiarOfertaPendiente, manejarRespuestaTextoActivacion,
  textoAChequeoId, manejarRespuestaAngel,
} from './angelGuardianEngine.js';
...
export async function procesarMensaje(telefono, nombreContacto, mensajeTexto) {
  try {
    ...
    // 2.65 ¿El Ángel Guardián está activo y el usuario avisa que ya llegó/
    // está bien? Se cierra acá, antes de Claude — no tiene sentido clasificar
    // "ya llegué a casa" como una intención cualquiera mientras acompañamos.
    if (angelEstaActivo(usuario) && esMensajeCierreAngel(mensajeTexto)) {
      guardarMensaje(usuario.id, 'user', mensajeTexto, { tipo_mensaje: 'texto' })
        .catch(e => console.error('Error guardando mensaje usuario:', e.message));
      await cerrarAngelGuardian(usuario, telefono, { silencioso: true });
      const final = 'Qué bueno que llegaste bien 🙌 Que descanses, aquí estaré el próximo sábado.';
      guardarMensaje(usuario.id, 'assistant', final, { intencion: 'angel_guardian_cierre' }).catch(() => {});
      return final;
    }

    // 2.66 Ángel Guardián: oferta de activación pendiente respondida por TEXTO
    // ("sí"/"no"), para quien no toca el botón (mucha gente escribe la respuesta).
    if (hayOfertaPendiente(usuario)) {
      const rta = await manejarRespuestaTextoActivacion(usuario, telefono, mensajeTexto);
      if (rta !== null) {
        guardarMensaje(usuario.id, 'user', mensajeTexto, { tipo_mensaje: 'texto' }).catch(() => {});
        guardarMensaje(usuario.id, 'assistant', rta, { intencion: 'angel_guardian_activacion' }).catch(() => {});
        return rta;
      }
      // No fue un sí/no claro: se suelta la oferta y el mensaje sigue el flujo normal.
      await limpiarOfertaPendiente(usuario);
    }

    // 2.67 Ángel Guardián activo: respuesta a un chequeo escrita por TEXTO
    // ("bien"/"más o menos"/"mal"), equivalente a tocar el botón.
    if (angelEstaActivo(usuario)) {
      const chequeoId = textoAChequeoId(mensajeTexto);
      if (chequeoId) {
        const rta = await manejarRespuestaAngel(usuario, telefono, chequeoId);
        guardarMensaje(usuario.id, 'user', mensajeTexto, { tipo_mensaje: 'texto' }).catch(() => {});
        if (rta) guardarMensaje(usuario.id, 'assistant', rta, { intencion: 'angel_guardian_chequeo' }).catch(() => {});
        return rta;
      }
    }

    // 2.68 "Salida"/"salidita" escrito en el chat (no todos tocan el botón del
    // sábado) → ofrecer activar el Ángel Guardián.
    if (!angelEstaActivo(usuario) && esMensajeSalida(mensajeTexto)) {
      guardarMensaje(usuario.id, 'user', mensajeTexto, { tipo_mensaje: 'texto' }).catch(() => {});
      await ofrecerAngelGuardian(usuario, telefono);
      guardarMensaje(usuario.id, 'assistant', '[oferta ángel guardián]', { intencion: 'angel_guardian_oferta' }).catch(() => {});
      return null;
    }
