...
// Muy conservadora a propósito: solo debe disparar cuando el usuario
// está presentando explícitamente a alguien en ESTE mensaje, nunca al
// mencionar o preguntar por alguien de pasada.
export async function detectarDatosPasivos(usuarioId, mensaje) {
  const detection = await anthropic.messages.create({
    ...
    messages: [{
      role: 'user',
      content: `Analiza este mensaje: "${mensaje}".

Detecta ÚNICAMENTE si el usuario está presentando/introduciendo explícitamente a una persona nueva con su relación, en ESTE mensaje (ej: "mi mamá se llama María", "mi hermano Joel me ayudó hoy").

NO detectes si:
- El usuario solo pregunta por alguien ("¿quién es Joel?", "¿sabes de Juan?")
- Menciona un nombre sin relación clara ni contexto de presentación
- Es ambiguo o dudoso

Responde SOLO en JSON:
{"detectado": true, "tipo": "persona", "valor": "nombre", "relacion": "mamá/hermano/amigo/etc", "confianza": 0.0-1.0}
o
{"detectado": false}

Usa "detectado": true SOLO si confianza >= 0.85. Ante la duda, responde {"detectado": false}.`,
    }],
  });
  ...
  try {
    ...
    if (parsed.detectado && (parsed.confianza == null || parsed.confianza < 0.85)) {
