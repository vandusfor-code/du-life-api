...
export async function aplicarPagoYFormatear(prestamo, monto, nombrePrestamista) {
  const resultado = await procesarPago(prestamo.id, monto);
  if (!resultado) return 'No pude registrar el pago.';

  // Aviso al deudor de que su pago ya quedó confirmado — solo si el usuario
  // activó el recordatorio para este préstamo (ahí quedó guardado su
  // WhatsApp). Nunca bloquea la respuesta al prestamista si falla el envío.
  if (prestamo.recordatorio_pago_activo && prestamo.telefono_deudor) {
    enviarPlantilla(prestamo.telefono_deudor, 'confirmacion_pago_prestamo', {
      nombre: prestamo.nombre_deudor,
      prestamista: nombrePrestamista || 'tu prestamista',
    }).catch(() => {});
  }
